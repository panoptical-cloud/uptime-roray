...
export const Route = createFileRoute('/server-groups/add-server')({
...
function RouteComponent() {
    ...
    const mutation = useMutation(
        async (formData: { name: string; hostname: string; ip: string; group_id: number }) => {
    ...
    return (
        <>
            <div className="w-2/3 max-w-xl m-8 bg-muted/50 rounded-xl p-8">
                <h2 className="mb-8 text-4xl font-semibold ">Add new server to 'group_name'</h2>
                <form onSubmit={(e) => {
                    e.preventDefault()
                    e.stopPropagation()
                    form.handleSubmit()
